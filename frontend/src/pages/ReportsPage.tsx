...
function ReportsPage() {
  ...
  const [projectErrorMessage, setProjectErrorMessage] =
    useState("");
  const [reportListErrorMessage, setReportListErrorMessage] =
    useState("");
  ...
  useEffect(() => {
    ...
    const loadReports = async () => {
      ...
      try {
        const reportList =
          await getWeeklyReportsByProject(
  ...
  const updateFormField = <
    K extends keyof WeeklyReportFormState,
  >(
    field: K,
  ...
  const handleSubmit = async (
    ...
    const targetProgress = Number(
      reportForm.targetProgress,
    );
    const actualProgress = Number(
      reportForm.actualProgress,
    );
    ...
    if (
      ...
      setFormErrorMessage(
        "Yapılanlar ve yapılacaklar alanları zorunludur.",
      );
      return;
    }
  ...
  return (
    <Box>
      ...
      {!isProjectsLoading &&
        ...
        projects.length === 0 && (
          <Alert severity="info">
            Haftalık rapor oluşturabilmek için önce
            bir proje oluşturulmalıdır.
          </Alert>
        )}
        ...
        projects.length > 0 && (
          <>
            ...
            {!isReportsLoading &&
              ...
              reports.length === 0 && (
                <Alert severity="info">
                  Seçilen proje için henüz haftalık
                  rapor bulunmuyor.
                </Alert>
              )}
